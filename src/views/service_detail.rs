...
pub fn view_service_detail<'a>(
    ...
    let pid_row = widget::row()
        .push(widget::text(pid_label).width(Length::Fixed(120.0)))
        .push(widget::text(service.pid.map(|p| p.to_string()).unwrap_or_else(|| "—".to_string())))
        .spacing(spacing.space_s);
